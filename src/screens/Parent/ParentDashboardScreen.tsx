...
// YOUR ORIGINAL UI - ONLY DATA FETCHING CHANGED, NOTHING ELSE
...
// ─── Constants (YOUR ORIGINAL CONSTANTS) ───────────────────────────────────────────────
            ...
            <SafeAreaView>
              ...
              <View style={sk.summaryStrip}>
                {[0, 1, 2, 3].map((i) => (
                  <View
                    key={i}
                    style={{ flex: 1, alignItems: "center", gap: 0 }}
                  >
...
// ─── Child Detail Modal (YOUR ORIGINAL - updated to handle real data) ───────────────────────────────────────
function ChildDetailModal({
  ...
  const gradient = CHAR_GRADIENT[child.chosenCharacter] || [
    "#7C5CBF",
    "#5A3D9A",
  ];
  ...
  return (
    ...
    >
      ...
      <Animated.View
        style={[cdm.sheet, { transform: [{ translateY: slideY }] }]}
      >
        ...
        >
          <View style={cdm.blobTR} />
          <View style={cdm.blobBL} />
          <View style={cdm.heroRow}>
            ...
            >
              <Text style={{ fontSize: 32 }}>
                {CHAR_EMOJI[child.chosenCharacter]}
              </Text>
            </View>
            <View style={{ flex: 1, marginLeft: 14 }}>
              <Text style={cdm.heroName}>{child.nickname}</Text>
              <Text style={cdm.heroMeta}>
                Age {child.age} · {CHAR_LABELS[child.chosenCharacter]}
              </Text>
              <View style={cdm.levelBadge}>
                <Text style={cdm.levelBadgeText}>
                  {getScoreLabel(child.confidenceScore)}
                </Text>
              </View>
            </View>
        ...
        >
          <View style={cdm.card}>
            <View style={cdm.cardHeaderRow}>
              ...
              <View
                style={[
                  cdm.deltaPill,
                  { backgroundColor: weekDelta >= 0 ? "#E8FFF3" : "#FFE8E8" },
                ]}
              >
                <Ionicons
                  name={weekDelta >= 0 ? "trending-up" : "trending-down"}
                  size={13}
                  color={weekDelta >= 0 ? "#4CAF50" : "#FF3B30"}
                />
                <Text
                  style={[
                    cdm.deltaText,
            ...
            <MiniLineChart
              scores={child.weeklyScores || [0, 0, 0, 0, 0, 0, 0]}
              color={color}
            />
          </View>
          <LinearGradient
            colors={[color + "15", color + "05"]}
            style={cdm.lastTopicCard}
          >
            <View
              style={[cdm.lastTopicIcon, { backgroundColor: color + "25" }]}
            >
              <Text style={{ fontSize: 22 }}>💬</Text>
            </View>
            <View style={{ flex: 1, marginLeft: 14 }}>
              <Text style={cdm.lastTopicLabel}>Last Practiced Topic</Text>
              <Text style={cdm.lastTopicValue}>
                {child.lastTopic || "No sessions yet"}
              </Text>
              <Text style={cdm.lastTopicDate}>
                {child.lastSessionDate || "Never"}
              </Text>
            </View>
            <Text style={[cdm.lastTopicScore, { color }]}>
              {child.sessions?.[0]?.score ?? "—"}
            </Text>
          </LinearGradient>
          <View style={cdm.card}>
            <View style={cdm.cardHeaderRow}>
              ...
              <View style={[cdm.scorePill, { backgroundColor: color }]}>
                <Text style={cdm.scorePillText}>
                  {child.confidenceScore} / 100
                </Text>
              </View>
            </View>
            <View style={cdm.progressTrack}>
              <LinearGradient
                ...
                style={[
                  cdm.progressFill,
                  { width: `${child.confidenceScore}%` as any },
                ]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
              >
                <Text style={cdm.progressEmoji}>
                  {child.confidenceScore >= 80
                    ? "👑"
                    : child.confidenceScore >= 60
                      ? "🌟"
                      : child.confidenceScore >= 40
                        ? "🌱"
                        : "🐣"}
                </Text>
              </LinearGradient>
            ...
            <View style={cdm.milestones}>
              {[25, 50, 75, 100].map((m) => (
                <View
                  ...
                  style={[
                    cdm.milestone,
                    child.confidenceScore >= m && { backgroundColor: color },
                  ]}
                >
                  <Text
                    style={[
                      cdm.milestoneText,
                      child.confidenceScore >= m && { color: "white" },
                    ]}
                  >
                    {m}
                  </Text>
          ...
          <View style={cdm.card}>
            ...
            <View style={{ marginTop: 14 }}>
              {child.sessions?.map((sess: any, i: number) => {
                ...
                const sg = CHAR_GRADIENT[sess.character] || [
                  "#7C5CBF",
                  "#5A3D9A",
                ];
                const barColor =
                ...
                return (
                  <View
                    ...
                    style={[
                      cdm.sessRow,
                      i < child.sessions.length - 1 && cdm.sessRowBorder,
                    ]}
                  >
                    <LinearGradient colors={sg as any} style={cdm.sessCharDot}>
                      <Text style={{ fontSize: 18 }}>
                        {CHAR_EMOJI[sess.character]}
                      </Text>
                    </LinearGradient>
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <Text style={cdm.sessTopic}>{sess.topic}</Text>
                      <Text style={cdm.sessMeta}>
                        {sess.date} · {CHAR_LABELS[sess.character]} ·{" "}
                        {sess.duration}
                      </Text>
                    ...
                    >
                      <Text style={[cdm.sessScoreNum, { color: barColor }]}>
                        {sess.score}
                      </Text>
                    </View>
                  </View>
          ...
          >
            ...
            <Text style={cdm.deleteBtnText}>
              Delete {child.nickname}'s Data
            </Text>
          </TouchableOpacity>
        </ScrollView>
      </Animated.View>
      ...
      <TouchableOpacity onPress={onPress} activeOpacity={0.88}>
        <View style={cc.card}>
            ...
            <View style={{ flex: 1, marginLeft: 14 }}>
              ...
              >
                ...
                <View style={[cc.agePill, { backgroundColor: color + "18" }]}>
                  <Text style={[cc.agePillText, { color }]}>
                    Age {child.age}
                  </Text>
                </View>
              </View>
              <Text style={cc.lastSession}>
                {child.lastTopic} · {child.lastSessionDate}
              </Text>
              <View
                style={{
                  flexDirection: "row",
              ...
              >
                <Text style={[cc.charLabel, { color }]}>
                  {CHAR_LABELS[child.chosenCharacter]}
                </Text>
                <Text style={cc.sessCount}>{child.totalSessions} sessions</Text>
              </View>
            </View>
            <View style={cc.scoreWrap}>
              <Text style={[cc.scoreNum, { color: scoreColor }]}>
          ...
          <View style={cc.footer}>
            <View
              style={[cc.levelPill, { backgroundColor: scoreColor + "18" }]}
            >
              <Text style={[cc.levelPillText, { color: scoreColor }]}>
                {getScoreLabel(child.confidenceScore)}
              </Text>
            </View>
            <View
...
// ─── Main Screen (YOUR ORIGINAL - with REAL DATA) ──────────────────────────────────────────────
export default function ParentDashboardScreen({ navigation, route }: any) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      for (const childUid of childUids) {
        ...
        const sessions: FirestoreSession[] = sessionsSnap.docs.map(
          (doc) =>
            ({
              id: doc.id,
              ...doc.data(),
        ...
        const formattedSessions = sessions.map((s: FirestoreSession) => ({
          ...
          duration: s.sessionDuration
            ? `${Math.floor(s.sessionDuration / 60)} min`
            : "? min",
        }));
      ...
      if (parentId) {
        ...
        if (parentSnap.exists()) {
          ...
          const updatedList = currentList.filter(
            (uid: string) => uid !== childUid,
          );
          await updateDoc(parentRef, {
            linkedChildren: updatedList,
  ...
  const avgScore = children.length
    ? Math.round(
        children.reduce((s, c) => s + c.confidenceScore, 0) / children.length,
      )
    : 0;
  ...
  return (
    <View style={ms.root}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          <LinearGradient
            colors={["#7C5CBF", "#5A3D9A"]}
            style={ms.hero}
            start={{ x: 0.1, y: 0 }}
            end={{ x: 0.9, y: 1 }}
          >
            <View style={ms.blobTR} />
            <View style={ms.blobBL} />
            <SafeAreaView>
        ...
        >
          ...
          <Text style={ms.sectionSub}>
            Tap any card to view detailed progress & analytics
          </Text>
        </Animated.View>
        ...
        <Animated.View style={[ms.safetyCard, { opacity: cardAnim }]}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={ms.safetyText}>
              Voice recordings are deleted after transcription. Only nicknames
      ...
      <ChildDetailModal
        ...
        onDelete={() => deleteChildData(selectedChild?.uid)}
      />
      <ConfirmModal
        ...
        onConfirm={deleteAllChildrenData}
      />
    </View>
  );
}
