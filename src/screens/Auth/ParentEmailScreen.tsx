...
export default function ParentEmailScreen({ navigation, route }: any) {
  ...
  const [isFinishing, setIsFinishing] = useState(false); // Loader for Math Quiz
  
  // Safe param handling to fix "Cannot read property 'ageGroup' of undefined"
  ...
  const checkMath = () => {
    if (parseInt(answer) === quiz.result) {
      setIsFinishing(true); // Show loader on the "Unlock" button
      
      // Short delay for that smooth "StockVerse" feel
      setTimeout(() => {
        navigation.navigate("NameInput", { ageGroup });
      }, 1500);
    } else {
      setAnswer("");
      // Logic for wrong answer can go here (shake animation, toast, etc.)
    }
  };
        ...
        >
          ...
          <View style={styles.header}>
            <View style={styles.shieldIcon}>
              <Ionicons name="shield-checkmark" size={30} color="#7C5CBF" />
            </View>
            <Text style={styles.brandTitle}>GUFTAGU SECURE</Text>
          </View>
          ...
          <Animated.View style={[styles.cardContainer, { transform: [{ translateX: slideAnim }] }]}>
            ...
            {step === "EMAIL" && (
              <View style={styles.stepContent}>
                ...
                <Text style={styles.subtitle}>Enter your email to receive a secure access link for your child's profile.</Text>
                <TextInput
                  style={styles.input}
                  ...
                  autoCapitalize="none"
                />
                <TouchableOpacity style={styles.mainBtn} onPress={handleSendEmail}>
            ...
            {step === "WAITING" && (
              <View style={styles.stepContent}>
                ...
                <Text style={styles.subtitle}>We've sent a link to {email}. We'll move forward once you tap it!</Text>
              </View>
            )}
            ...
            {step === "QUIZ" && (
              <View style={styles.stepContent}>
                ...
                <Text style={styles.subtitle}>Solve this to prove you're the parent:</Text>
...
const styles = StyleSheet.create({
  ...
  brandTitle: { 
    ...
    color: '#7C5CBF', marginTop: 15, letterSpacing: 2 
  },
  cardContainer: { flex: 1, width: '100%' },
  stepContent: { width: '100%', alignItems: 'center' },
  title: { 
    fontFamily: 'Poppins-ExtraBold', fontSize: 28, 
    color: '#1A1A1A', textAlign: 'center' 
  },
  subtitle: { 
    fontFamily: 'Poppins-Medium', fontSize: 15, 
    color: '#777', textAlign: 'center', marginTop: 12, lineHeight: 22 
  },
  input: { 
    width: '100%', height: 65, backgroundColor: '#FFF', 
    borderRadius: 20, paddingHorizontal: 25, fontSize: 16, 
    fontFamily: 'Poppins-Medium', marginTop: 35, borderWidth: 1, borderColor: '#EEE'
  },
  mainBtn: { 
    width: '100%', height: 65, backgroundColor: '#7C5CBF', 
  ...
  quizBox: { 
    ...
    marginVertical: 40, backgroundColor: '#FFF', padding: 25, borderRadius: 30,
    elevation: 3, shadowOpacity: 0.05
  },
  mathText: { fontFamily: 'Poppins-ExtraBold', fontSize: 36, color: '#1A1A1A' },
  mathInput: { 
    width: 70, borderBottomWidth: 3, borderColor: '#7C5CBF', 
    fontSize: 36, fontFamily: 'Poppins-Bold', textAlign: 'center', color: '#7C5CBF'
  },
  goBtn: { 
    width: '100%', height: 65, backgroundColor: '#7C5CBF', 
